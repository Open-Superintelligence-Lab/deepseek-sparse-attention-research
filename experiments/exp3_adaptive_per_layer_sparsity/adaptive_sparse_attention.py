...
def create_sparsity_schedule(
    ...
    if schedule == SparsitySchedule.DENSE_BASELINE:
    ...
    elif schedule == SparsitySchedule.REVERSE_PROGRESSIVE:
        # Opposite hypothesis: Sparse early, dense late
        # Early: Aggressive (k=L/4)
        # Middle: Moderate (k=L/2)
        # Late: Dense (k=L)
        ratios = []
        early_cutoff = n_layers // 3
        middle_cutoff = 2 * n_layers // 3
        ...
        for i in range(n_layers):
            if i < early_cutoff:
                ...
                ratios.append(1.0)  # Dense
        description = "Reverse-Progressive: Early=L/4, Middle=L/2, Late=Dense"
